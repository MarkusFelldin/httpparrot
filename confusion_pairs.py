...
CONFUSION_PAIRS = [
    ...
    },
    {
        ...
        "category": "Redirect pairs",
        "tldr": "Both are permanent redirects, but 301 may change POST to GET while 308 guarantees the HTTP method is preserved.",
        "decision_tree": [
            {"question": "Does the client need to preserve the HTTP method (e.g., keep POST as POST)?", "yes": "308", "no": "301"},
            {"question": "Is this a simple page/URL move where GET is fine?", "yes": "301", "no": "308"},
            {"question": "Are you building a REST API where method preservation matters?", "yes": "308", "no": "301 is usually fine"},
        ],
        "examples": [
            {"scenario": "A blog moves from /old-blog to /new-blog. All traffic is GET requests.", "code": "301", "explanation": "GET requests don't have method preservation concerns \u2014 301 works perfectly."},
            {"scenario": "An API permanently moves POST /v1/orders to POST /v2/orders.", "code": "308", "explanation": "POST must stay POST \u2014 301 might change it to GET, losing the request body."},
            {"scenario": "A domain migration from old.com to new.com for a static website.", "code": "301", "explanation": "Static sites only serve GET \u2014 301 is the standard choice for domain moves."},
        ],
        "quiz": [
            {"scenario": "An API endpoint POST /api/submit permanently moves to /api/v2/submit.", "correct": "308", "wrong": "301"},
            {"scenario": "A marketing page /promo permanently redirects to /deals.", "correct": "301", "wrong": "308"},
            {"scenario": "A file upload endpoint PUT /upload/old permanently moves to /upload/new.", "correct": "308", "wrong": "301"},
        ],
    },
    {
        "slug": "503-vs-504",
    ...
    },
    {
        ...
        "category": "Success pairs",
        "tldr": "200 means the request succeeded and returns data; 201 means the request succeeded AND a new resource was created.",
        "decision_tree": [
            {"question": "Did the server create a new resource?", "yes": "201", "no": "200"},
            {"question": "Is this a GET request returning existing data?", "yes": "200", "no": "Check if resource was created"},
            {"question": "Should the response include a Location header pointing to the new resource?", "yes": "201 (with Location)", "no": "200"},
        ],
        "examples": [
            {"scenario": "A GET /api/users/42 returns the user's profile data.", "code": "200", "explanation": "The user already exists \u2014 we're just retrieving data, not creating anything."},
            {"scenario": "A POST /api/users creates a new user account and returns the created user object.", "code": "201", "explanation": "A new resource was created \u2014 the server should also include a Location header pointing to /api/users/43."},
            {"scenario": "A PUT /api/users/42 updates the user's email address.", "code": "200", "explanation": "The user was updated, not created \u2014 200 is correct for modifications to existing resources."},
        ],
        "quiz": [
            {"scenario": "Your API receives POST /api/orders and creates order #1234.", "correct": "201", "wrong": "200"},
            {"scenario": "A client sends GET /api/products?category=shoes and gets a list back.", "correct": "200", "wrong": "201"},
            {"scenario": "A PUT /api/config updates system settings successfully.", "correct": "200", "wrong": "201"},
        ],
    },
]
