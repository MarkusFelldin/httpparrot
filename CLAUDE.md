...
- 943 tests across test_app.py (925) and test_cli.py (18) — heavily consolidated via parametrize + batch assertions
- Coverage: 97%+ of index.py

## Architecture

- `index.py` -- main Flask app with all routes (~2287 lines)
...
- `scenarios.py` -- 72 practice scenarios (including 5 security category) with difficulty, options, and explanations
- `debug_exercises.py` -- 38 debug exercises with broken HTTP exchanges and bugs to find
- `confusion_pairs.py` -- 22 confusion pair lesson definitions with slugs, categories, and content
...
- `templates/` -- 41 Jinja2 templates (base.html has XP system, ranks, badge JS, toast system, and XP multiplier)
